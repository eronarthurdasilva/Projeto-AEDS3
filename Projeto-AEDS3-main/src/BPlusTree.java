...
public class BPlusTree {
    ...
    private void split(BPlusNode node) {
        ...
        // Só copia se houver elementos após o meio
        if (midIndex + 1 <= node.keys.size()) {
            if (midIndex + 1 < node.keys.size()) {
                newNode.keys.addAll(node.keys.subList(midIndex + 1, node.keys.size()));
            }
            if (midIndex + 1 < node.values.size()) {
                newNode.values.addAll(node.values.subList(midIndex + 1, node.values.size()));
            }
        }

        // Para nós internos, também move os filhos
        if (!node.isLeaf) {
            if (midIndex + 1 < node.children.size()) {
        ...
        // Limpa as listas do nó original a partir do meio
        node.keys.subList(midIndex, node.keys.size()).clear();
        if (midIndex < node.values.size()) {
